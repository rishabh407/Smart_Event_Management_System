...
const EditEvent = () => {
  ...
  // The original start date fetched from server — used as fallback min for start picker
  // so the picker doesn't block the pre-filled value even if it's in the past
  ...
  // ── Min for start picker:
  //    Use the earlier of (originalStartDate, nowISO) so the pre-filled value
  //    is always selectable, but user can't pick anything before the original.
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <DateTimePicker
            ...
            // isStart=false for edit — no "today" banner since original date may be past
            isStart={false}
            required
          />
          <DateTimePicker
